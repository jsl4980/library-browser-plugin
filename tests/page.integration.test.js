...
test("TC-US2-AMAZON-LEGACY-ASIN-URL renders on legacy Amazon ASIN URLs", async () => {
...
test("TC-US3-EXACT-RELATED-SECTIONS renders exact and related blocks with distinct catalog links", async () => {
  ...
  const keywordUrl = "https://catalog.onlib.org/polaris/view.aspx?keyword=The%20Testable%20Library%20Ada%20Example";
  const formatsExact = [
    { bucket: "physical_book", label: "Book", availability: "available_now", hint: "Available now" },
    { bucket: "ebook", label: "E-book", availability: "hold_available", hint: "Hold or request" }
  ];
...
test("TC-US3-RELATED-SECTION-ONLY shows related heading and link when there is no ISBN block", async () => {
  const testCase = pageCases.get("TC-US3-RELATED-SECTION-ONLY");
  const keywordUrl = "https://catalog.onlib.org/polaris/view.aspx?keyword=Fallback%20Catalog%20Search%20Taylor%20Query";
  const harness = createPageHarness({
    html: loadPageFixture("goodreads-book.html"),
    url: "https://www.goodreads.com/book/show/1-the-testable-library",
    runtimeResult: {
